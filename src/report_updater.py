...
def append_family_ids_to_report(
    ...
    """Append new family IDs to B and AL columns of 報告書. Returns rows used.

    Skips IDs that are already present in the B column.
    Raises if target cells are merged.
    """
    from openpyxl.cell.cell import MergedCell

    # Collect existing IDs to avoid duplicates
    b_idx = column_letter_to_index(b_column)
    al_idx = column_letter_to_index(al_column)
    existing = set()
    ...
    for fid in family_ids:
        ...
        if isinstance(b_cell, MergedCell) or isinstance(al_cell, MergedCell):
            raise RuntimeError(
                f"Cannot append to row {cursor}: cell is part of a merged range. "
                "Check 報告書 sheet's merged cells and adjust data_start_row."
            )
        b_cell.value = fid
        # Existing pattern in 報告書 sets AL column as =B{row} formula
